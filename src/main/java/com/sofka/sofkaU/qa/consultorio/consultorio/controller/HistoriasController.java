...
public class HistoriasController {
    ...
    public ResponseEntity actualizar(@PathVariable String cedula, @RequestBody HistoriaMedica historiaMedica) {
        historiaMedicaIService.actualizar(pacienteRepository.getPacientes().stream()
                .filter(h -> h.getCedula().equalsIgnoreCase(cedula))
                .findFirst().get(),historiaMedica);
        return new ResponseEntity(historiaMedica,HttpStatus.ACCEPTED);
    }
}
